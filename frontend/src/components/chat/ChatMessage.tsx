...
export default function ChatMessage({ role, content, isLast }: Props) {
  ...
  const displayText = isString ? (isBot && isLast ? displayed || content : content) : content

  // Estilo de burbuja por rol.
  const bubble = isUser
  ...
  const align = isUser ? 'justify-end' : isSystem ? 'justify-center' : 'justify-start'

  return (
    <motion.div
      initial={{ opacity: 0, y: reduce ? 0 : 12, x: reduce ? 0 : isUser ? 10 : -10 }}
      animate={{ opacity: 1, y: 0, x: 0 }}
      transition={{ duration: reduce ? 0 : 0.28, ease: [0.22, 1, 0.36, 1] }}
      className={`flex ${align} gap-2.5 px-4 ${isLast ? '' : 'mb-3'}`}
      role="listitem"
      aria-label={`Mensaje de ${isAssistant ? 'AnemIA' : isUser ? 'usuario' : isSystem ? 'sistema' : 'agente'}`}
    >
      {!isUser && !isSystem && (
      ...
      <div className={`min-w-0 ${isUser ? 'flex flex-col items-end' : ''} max-w-[82%]`}>
        ...
        <div className={`px-4 py-3 text-sm leading-relaxed ${bubble}`}>
          ...
          {!done && isBot && isLast && (
            <span className="ml-0.5 inline-block h-4 w-1.5 animate-pulse bg-teal-500 align-text-bottom" aria-hidden="true" />
          )}
        </div>
      </div>

      {isUser && (
        <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-slate-700 text-white shadow-sm" aria-hidden="true">
          <User size={14} />
        </div>
      )}
    </motion.div>
